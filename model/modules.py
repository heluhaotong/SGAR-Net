...
class SFA(nn.Module):
    def __init__(self, in_channels, out_channels, scale_factors=[0.5, 1, 2, 4], fuse_type="sum"):
        ...
        self.lateral_convs = nn.ModuleList([
            ConvBatchNormReLU(out_channels, out_channels, 1, 1, 0, 1, leaky=True) for _ in range(4)
        ])

        self.output_convs = nn.ModuleList([
            ConvBatchNormReLU(out_channels, out_channels, 3, 1, 1, 1, leaky=True) for _ in range(4)
        ])
