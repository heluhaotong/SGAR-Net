...
class enc_tf_lang(nn.Module):

    def __init__(self, input_1, input_2, hidden_dim, head_num, dropout=0.1):
        super(enc_tf_lang, self).__init__()
        self.pos_embedding_1 = PositionEmbeddingSine(input_1, normalize=True)
        self.pos_embedding_2 = PositionEmbeddingSine(input_2, normalize=True)
        ...
        self.activation = nn.ReLU(hidden_dim)
        self.dropout = nn.Dropout(dropout)

    # @get_local("weights")
    def forward(self, lang_input,vision_input):
        decoder_embed_lang = lang_input
        decoder_embed_vis = vision_input
        q_inp = F.relu(self.dense_q(decoder_embed_lang).permute(1, 0, 2))
        k_inp = F.relu(self.dense_k(decoder_embed_vis).permute(1, 0, 2))
        v_inp = F.relu(self.dense_v(decoder_embed_vis).permute(1, 0, 2))
        decoded_layer, weights = self.self_attn(q_inp, k_inp, v_inp)

        decoded_layer = decoded_layer.permute(1, 0, 2)
        add_layer = decoded_layer + lang_input
...
class TransformerCrossAttentionLayer(nn.Module):
    ...
    def forward(self, tgt, memory, memory_mask=None, memory_key_padding_mask=None):
        ...
        tgt2 = tgt2 + self.dropout1(attn_output)
        # Step 3: Feedforward + Residual + Norm
        residual = tgt
        tgt2 = self.linear2(tgt2)
        tgt2 = self.dropout2(tgt2)
        tgt2 = self.norm2(tgt2 + residual)
        tgt2 = self.Relu(tgt2)
        return tgt2

#论文中Local Visual Attention 模块
